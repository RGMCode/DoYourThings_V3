...
struct DoYourThingEditView: View {
    ...
    init(viewModel: DoYourThingViewModel, task: DoYourThing, context: NSManagedObjectContext) {
                            ...
                            Picker("", selection: $selectedCategory) {
                                ForEach(viewModel.categories, id: \.self) { cat in
                                    Text(cat.originalName ?? "")
                                        .foregroundColor(Color(hex: cat.colorHex ?? "#000000") ?? .primary)
    ...
    private func saveTask() {
        ...
        viewModel.fetchDYT()  // Aktualisiere die Aufgabenliste im ViewModel
        presentationMode.wrappedValue.dismiss()
    }
